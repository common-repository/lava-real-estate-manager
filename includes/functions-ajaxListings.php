<?php
add_action( 'wp_ajax_lava_realestate_listing' , 'lava_ajax_realestate_listing' );
add_action( 'wp_ajax_nopriv_lava_realestate_listing', 'lava_ajax_realestate_listing' );

function lava_ajax_realestate_listing() {
	...
	if( $lava_listing_posts->have_posts() )
	{
		while( $lava_listing_posts->have_posts() )
		{
			...
			if( !empty( $arrGetTaxonomies ) ) : foreach( $arrGetTaxonomies as $key => $meta ) {
				$arrTaxonomies[ $key ]	= lava_realestate_featured_terms( $key, get_the_ID(), false );
			} endif;

			$attachment_id				= apply_filters( 'lava_realestate_listing_featured_image_id', get_post_thumbnail_id(), get_the_ID() );
